...
def remove_all_files(path, zip_path, outputDir):
...
def convert_excel_to_csv(folder_path):
    exclude_sheet_substrings = ["ΣΤΑΤΙΣΤΙΚΑ".lower(),
                                "ΣΤΑΤ".lower()]
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".xlsx"):
            ...
            excel_sheets = pd.read_excel(input_path,
                                         sheet_name=None)

            for sheet_name in list(excel_sheets.keys()):
                if any(substring in sheet_name.lower() for
                       substring in exclude_sheet_substrings):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method delete_AA : {e},{filename}")
...
def delete_AA_ROHS(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method delete_AA_ROHS : {e},{filename}")
...
def normalize_type(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method normalize_type : {e},{filename}")
...
def delete_mitronimo(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            df = pd.read_csv(input_path, encoding='utf-8')
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method delete_mitronimo : {e},{filename}")
...
def delete_pinakas(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method delete_pinakas : {e},{filename}")
...
def check_moria_pinaka(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
...
def normalize_perioxi_topothetisis(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                if 'ΠΕΡΙΟΧΗ ΠΡΟΣΛΗΨΗΣ' in df.columns:
                    df['ΠΕΡΙΟΧΗ ΤΟΠΟΘΕΤΗΣΗΣ'] = df['ΠΕΡΙΟΧΗ ΠΡΟΣΛΗΨΗΣ']
                    df = df.drop(['ΠΕΡΙΟΧΗ ΠΡΟΣΛΗΨΗΣ'], axis=1)
            except Exception as e:
                print(
                    f"An error occurred during preprocessing on method normalize_normalize_perioxi_topothetisis : {e},{filename}")
            ...
            replacement_mapping = {
                ...
                "ΣΔΕΥ στο ΚΕΔΑΣΥ ΕΥΒΟΙΑΣ": "Εύβοιας",
                ...
                "ΔΥΤ. ΑΤΤΙΚΗΣ": "Δυτ. Αττικής",

                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ ΑΜΠΕΛΟΚΗΠΩΝ ΘΕΣΣΑΛΟΝΙΚΗΣ ΚΑΙ ΚΑΛΛΙΤΕΧΝΙΚΟ ΛΥΚΕΙΟ ΣΤΑΥΡΟΥΠΟΛΗΣ ΘΕΣΣΑΛΟΝΙΚΗΣ": "Καλλιτεχνικό Γυμνάσιο Αμπελοκήπων Θεσσαλονίκης Και Καλλιτεχνικό Λύκειο Σταυρούπολης Θεσσαλονίκης",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΣΧΟΛΕΙΟ ΓΕΡΑΚΑ ΑΤΤΙΚΗΣ": "Καλλιτεχνικό Σχολείο Γέρακα Αττικής",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ ΠΕΡΙΣΤΕΡΙΟΥ": "Καλλιτεχνικό Γυμνάσιο Περιστερίου",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ ΚΕΡΑΤΣΙΝΙΟΥ ΔΡΑΠΕΤΣΩΝΑΣ": "Καλλιτεχνικό Γυμνάσιο Κερατσινίου Δραπετσώνας",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΣΧΟΛΕΙΟ Ηρακλείου ΚΡΗΤΗΣ": "Καλλιτεχνικό Σχολείο Ηρακλείου Κρήτης",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ ΑΘΗΝΑΣ": "Καλλιτεχνικό Γυμνάσιο Αθήνας",
                "ΕΚΚΛ/ΚΟ ΛΥΚΕΙΟΓΥΜΝΑΣΙΟ": "Εκκ/κο Λυκειογυμνάσιο",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ ΜΕΣΟΛΟΓΓΙΟΥ": "Καλλιτεχνικό Γυμνάσιο Μεσολογγίου",
                "ΚΑΛΛΙΤΕΧΝΙΚΟ ΓΥΜΝΑΣΙΟ Αργολίδας": "Καλλιτεχνικό Γυμνάσιο Αργολίδας",
            ...
            try:
                ...
                df['ΠΕΡΙΟΧΗ ΤΟΠΟΘΕΤΗΣΗΣ'] = df['ΠΕΡΙΟΧΗ ΤΟΠΟΘΕΤΗΣΗΣ'].str.replace('Α ΑΝΑΤ. ΑΤΤΙΚΗΣ', 'Α Ανατ. Αττικής')
            except Exception as e:
                print(
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(
                    f"An error occurred during preprocessing on method normalize_klados_values: {e},{filename}")
...
def delete_periferia(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method delete_periferia : {e},{filename}")
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method create_sxolia : {e},{filename}")
...
def add_mousika_organa_to_sxolia(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                df['ΣΧΟΛΙΑ'] = df['ΣΧΟΛΙΑ'] + ', ' + df['ΜΟΥΣΙΚΗ ΕΙΔΙΚΕΥΣΗ']
                df.drop(columns=['ΜΟΥΣΙΚΗ ΕΙΔΙΚΕΥΣΗ'], inplace=True)
                df.drop(columns=['ΩΡΑΡΙΟ'], inplace=True)

            except Exception as e:
                print(
                    f"An error occurred during preprocessing on method add_mousika_organa_to_sxolia : {e},{filename}")
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method create_sxoliko_etos : {e},{filename}")
            ...
            if year_from_filename:
                try:
                    ...
                    print(
                        f"An error occurred during preprocessing on method create_sxoliko_etos : {e},{filename}, or date {date_object}")
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(f"An error occurred during preprocessing on method normalize_columns : {e},{filename}")
...
def normalize_columns(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            # Construct the full path of the input CSV file
            input_path = os.path.join(folder_path, filename)
            ...
            try:
                ...
                print(
                    f"An error occurred during preprocessing on method normalize_columns ΣΧΟΛΙΚΗ ΜΟΝΑΔΑ ΤΟΠΟΘΕΤΗΣΗΣ: {e},{filename}")

            try:
                df = df.drop('Τύπος Πρόσληψης', axis=1)
            except Exception as e:
                print(
                    f"An error occurred during preprocessing on method normalize_columns Τύπος Πρόσληψης : {e},{filename}")

            try:
                df = df.drop('ΤΡΙΤΕΚΝΟΣ', axis=1)
            except Exception as e:
                print(f"An error occurred during preprocessing on method normalize_columns ΤΡΙΤΕΚΝΟΣ: {e},{filename}")
...
def normalize_all_columns_names(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            input_path = os.path.join(folder_path, filename)
            column_mapping = {
...
def full_outer_join_csv_files(folder_path, final_output_path):
    result_df = None
    output_path = None
    try:
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith(".csv"):
                ...
                current_df['Seira_Pinaka'] = pd.to_numeric(
                    current_df['Seira_Pinaka'], errors='coerce').fillna(0).astype(int)
                ...
                if result_df is None:
                    ...
                    try:
                        ...
                        print(f"An error occurred during merge in file {filename}: {e}")
        ...
        if result_df is not None:
            ...
            print("No CSV files found or merged.")

    except Exception as e:
        print(f"An error occurred during preprocessing in method full_outer_join_csv_files: {e}")

    return output_path


def remove_inferior_duplicates(csv_path):
    df = pd.read_csv(csv_path, encoding='utf-8', dtype=str).fillna('')

    def is_missing(val):
        return str(val).strip().lower() in {'', '-', 'null', 'nan', 'κενό'}

    comparison_columns = [
        'Klados',
        'Seira_Pinaka',
        'Moria_Pinaka',
        'Perioxh_Topothethshs',
        'Dieytynsh_Ekpaideyshs',
        'Sxolia'
    ]
    ...
    identity_columns = [
        "Typos", "Eponymo", "Onoma", "Patronymo",
        "Hmeromnia", "Etos"
    ]

    grouped = df.groupby(identity_columns, sort=False)
    cleaned_rows = []
    removed_rows = []
    ...
    for _, group in grouped:
        if len(group) == 1:
            ...
            group = group.copy()
            group['__score'] = group[comparison_columns].apply(
                lambda row: sum(not is_missing(val) for val in row), axis=1
            )
            group_sorted = group.sort_values('__score', ascending=False)
            best_row = group_sorted.iloc[0]
            dropped_rows = group_sorted.iloc[1:]

            for _, row in dropped_rows.iterrows():
                removed_rows.append(row.drop('__score'))

            cleaned_rows.append(best_row.drop('__score'))

    cleaned_df = pd.DataFrame(cleaned_rows)

    ordered_columns = [
        "Typos", "Eponymo", "Onoma", "Patronymo", "Klados",
        "Seira_Pinaka", "Moria_Pinaka", "Perioxh_Topothethshs",
        "Dieytynsh_Ekpaideyshs", "Hmeromnia", "Etos", "Sxolia"
    ]
    ...
    cleaned_path = os.path.join(os.path.dirname(csv_path), cleaned_filename)
    cleaned_df.to_csv(cleaned_path, index=False, encoding='utf-8', na_rep='NULL')

    if removed_rows:
        removed_df = pd.DataFrame(removed_rows)
        removed_df = removed_df[ordered_columns]
        current_date = date.today().strftime("%Y-%m-%d")
        random_number = random.randint(10000, 99999)
        removed_filename = f"removed_duplicates-{current_date}-{random_number}.csv"
        removed_path = os.path.join(os.path.dirname(csv_path), removed_filename)
        # removed_df.to_csv(removed_path, index=False, encoding='utf-8', na_rep='NULL')

    try:
        os.remove(csv_path)
    except Exception as e:
        print(f"Failed to delete merged file {csv_path}: {e}")
